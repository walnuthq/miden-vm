...
impl Deserializable for DebugFileInfo {
    fn read_from<R: ByteReader>(source: &mut R) -> Result<Self, DeserializationError> {
        ...
        let directory_idx = if has_directory {
            Some(source.read_u32()?)
        } else {
            None
        };
...
mod tests {
    ...
    fn test_debug_info_section_roundtrip() {
        ...
        // Add a pointer type
        section.add_type(DebugTypeInfo::Pointer {
            pointee_type_idx: i32_type_idx,
        });
        ...
        func.add_variable(
            DebugVariableInfo::new(x_idx, i32_type_idx, 10, 5).with_arg_index(1),
        );
    ...
    fn test_file_info_with_checksum_roundtrip() {
        let file = DebugFileInfo::new(0)
            .with_directory(1)
            .with_checksum([42u8; 32]);
        roundtrip(&file);
    }
    ...
    fn test_function_with_mast_root_roundtrip() {
        ...
        func.add_variable(
            DebugVariableInfo::new(0, 0, 5, 10)
                .with_arg_index(1)
                .with_scope_depth(2),
        );
